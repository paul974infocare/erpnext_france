...
def get_down_payment_item_default(item_code, company):
        item = frappe.get_doc("Item", item_code)

        if not item or len(item.item_defaults) == 0:
                frappe.throw(_(f"Item {item_code} does not has default settings"))
        ...
        frappe.throw(_(f"Missing Default Income Account On Item {item_code}"))
...
def get_advance_entries(doc):
	...
	party_account = get_party_account(party_type, party=party, company=doc["company"], include_advance=True)
	...
	payment_entries = get_advance_payment_entries(party_type, party, party_account, order_doctype, order_list)
		...
		for x in lst:
			for dim in active_dimensions:
				if doc.get(dim.fieldname):
					x.update({dim.fieldname: doc.get(dim.fieldname)})
...
def set_total_advance_paid(doc):
	...
	if advance:
		...
		formatted_advance_paid = fmt_money(
			advance_paid,
			precision=doc.precision("advance_paid"),
		...
		formatted_order_total = fmt_money(
			order_total,
			precision=doc.precision(precision),
			currency=advance.account_currency,
		)
...
def make_exchange_gain_loss_gl_entries(doc, gl_entries):
	...
	for d in doc.get("advances"):
		...
		if account_currency != doc.company_currency:
			frappe.throw(_("Currency for {0} must be {1}").format(gain_loss_account, doc.company_currency))
